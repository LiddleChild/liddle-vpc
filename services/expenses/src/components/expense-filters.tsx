...
export function ExpenseFilters({ accountNames, categories, tags, value, onApply }: ExpenseFiltersProps) {
  ...
  return <section className="filter-page" role="dialog" aria-modal="true">
        ...
        <div className="filter-section"><span className="filter-label">Accounts</span><div className="tag-options">{accountNames.map((account) => { const selected = draft.accounts.includes(account); return <button className={`tag-option ${selected ? "selected" : ""}`} key={account} onClick={() => updateFilters({ ...draft, accounts: toggle(draft.accounts, account) })} aria-pressed={selected}>{account}</button> })}</div></div>
